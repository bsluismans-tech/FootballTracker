          ...
          return (
            <div key={p.id} className="p-2 pl-3 flex items-center gap-3">
              ...
              <div className="flex gap-1.5 flex-[3.5]">
                <button onClick={() => handleButtonClick(() => onUpdateQuarter({ goals: [...quarter.goals, p.id] }))} onTouchStart={() => handlePressStart(() => decrementStat(activeQuarterIdx, 'goals', p.id))} onTouchEnd={handlePressEnd} className={`flex-1 py-3 rounded-lg flex items-center justify-center gap-1 transition-all active:scale-95 ${gc > 0 ? 'bg-green-600 text-white shadow-sm' : 'bg-green-50 text-green-700 border border-green-100'}`}><Target size={14}/><span className="text-[9px] font-black uppercase">Goal {gc}</span></button>
                <button onClick={() => handleButtonClick(() => onUpdateQuarter({ assists: [...((quarter as any).assists || []), p.id] }))} onTouchStart={() => handlePressStart(() => decrementStat(activeQuarterIdx, 'assists', p.id))} onTouchEnd={handlePressEnd} className={`flex-1 py-3 rounded-lg flex items-center justify-center gap-1 transition-all active:scale-95 ${ac > 0 ? 'bg-yellow-500 text-white shadow-sm' : 'bg-yellow-50 text-yellow-700 border border-yellow-100'}`}><Handshake size={14}/><span className="text-[9px] font-black uppercase">Assis {ac}</span></button>
                <button onClick={() => handleButtonClick(() => onUpdateQuarter({ tackles: [...((quarter as any).tackles || []), p.id] }))} onTouchStart={() => handlePressStart(() => decrementStat(activeQuarterIdx, 'tackles', p.id))} onTouchEnd={handlePressEnd} className={`flex-1 py-3 rounded-lg flex items-center justify-center gap-1 transition-all active:scale-95 ${tc > 0 ? 'bg-blue-600 text-white shadow-sm' : 'bg-blue-50 text-blue-700 border border-blue-100'}`}><Axe size={14}/><span className="text-[9px] font-black uppercase">Tack {tc}</span></button>
              </div>
            </div>
          );
        })}
